...
Creating the file /tmp/etoro_kill_switch immediately forces CRITICAL regime.
Removing the file allows normal regime detection to resume on next Risk Worker run.

Usage:
    # Activate:
    echo 'Emergency stop — manual' > /tmp/etoro_kill_switch

    # Deactivate:
    rm /tmp/etoro_kill_switch
...
KILL_SWITCH_FILE = Path('/tmp/etoro_kill_switch')
...
def activate(reason: str = 'Manual kill switch') -> None:
    ...
    KILL_SWITCH_FILE.write_text(reason)
